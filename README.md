...
### 📚 [RAG Chatbot - Multimodal, Multi-Provider](https://github.com/vnaveenb/rag-fastapi-stack-chatbot)
Doc-grounded chatbot with **strict per-user isolation** and answers grounded only in uploaded content (web/model-knowledge toggles off by default). Docling + Tesseract OCR ingestion, hybrid dense+BM25 retrieval fused via RRF, Cohere cross-encoder reranking, semantic caching, streaming citations, and an LLM-as-judge eval harness, fully Dockerized.

`FastAPI` `Celery` `pgvector` `Redis` `React` `Docling`
...
### 🖥️ [omv-mcp - OpenMediaVault MCP Server](https://github.com/vnaveenb/openmediavault-mcp-ssh)
PyPI-published MCP server (`uvx omv-mcp setup`) exposing **48 tools**: system, storage/SMART health, Docker, and Portainer stacks, to any MCP-capable AI client (Claude, Copilot, Cursor). Runs over a single persistent SSH connection with passwordless key setup, trust-on-first-use host pinning, and `confirm=True` gating on every destructive tool.

`MCP` `Python` `uv` `SSH` `Portainer`
